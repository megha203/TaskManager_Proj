...
<div class="card">
  ...
  <div class="card-footer">
    ...
    data-bs-target="#existingTask" onclick="loadDataModal.apply(this, arguments)" id="openTask" name=${taskData.id}>Open Task</button>
  </div>
</div>
</div>`;
};
...
const loadDataModal = (event) => {

  const targetID = event.target.getAttribute("name");
  const getTask = globalTaskData.filter((task) => task.id===targetID);
  console.log(getTask)
  document.getElementById("task_Title").innerHTML=getTask[0].title;  
  document.getElementById("task_Image").src=getTask[0].image;
  document.getElementById("task_Type").innerHTML="Type: " + getTask[0].type;  
  document.getElementById("task_Description").innerHTML=getTask[0].description;
}
